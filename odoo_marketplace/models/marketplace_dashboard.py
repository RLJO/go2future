...
class marketplace_dashboard(models.Model):
    ...
    def _get_pending_count(self):
        ...
        if self.state == 'order':
            ...
            if self.is_seller:
                obj = self.env['sale.order.line'].search(
                    [('state','!=','draft'), ('marketplace_seller_id', '=',user_obj.partner_id.id), ('marketplace_state', '=', 'pending')])
            else:
                obj = self.env['sale.order.line'].search(
                    [('state','!=','draft'), ('marketplace_seller_id', '!=', False), ('marketplace_state', '=', 'pending')])
            self.count_product_pending = len(obj)
        if self.state == 'payment':
            obj = self.env['seller.payment'].search(
    ...
    def _get_new_count(self):
        if self.state == 'order':
            ...
            self.count_product_new = len(obj)
