        ...
        for rec in self:
            if rec.payment_mode == "order_paid":
                if rec.memo:
                    ...
                    if stock_move_objs:
                        quantity_done = sum(stock_move_objs.mapped('quantity_done'))
                        quantity_ordered = sum(rec.invoice_line_ids.mapped('quantity'))
        ...
        for rec in self:
            if vals.get("payable_amount", False) or vals.get("payment_type", False):
                ...
                if payment_type == 'dr':
                    ...
                    vals.update({'payable_amount': abs(payable_amount)})
        return super(SellerPayment, self).write(vals)
    ...
    def do_Confirm(self):
        for rec in self:
            ...
            if created_invoice_obj:
                ...
                pro_obj = self.env["product.product"].sudo().browse(product_id)
                company_id = rec.seller_id.company_id.id
                taxes = pro_obj.taxes_id.filtered(lambda r: r.company_id.id == company_id)
                taxes = created_invoice_obj.fiscal_position_id.map_tax(taxes, pro_obj, created_invoice_obj.partner_id)
                if taxes:
                    line["invoice_line_tax_ids"] = [(6, 0, taxes.ids)]

                ctx = self.env.context.copy()
                ctx.update({'journal_id': journal_ids[0].id})
                invoice_line_obj = self.env["account.invoice.line"].with_context(ctx).create(line)
                created_invoice_obj.action_invoice_open()
                if taxes:
                    created_invoice_obj._onchange_invoice_line_ids()
