...
class ResPartner(models.Model):
    ...
    internal_categ = fields.Many2one("product.category", string="Internal Category", copy=False, help="Seller products will belong to defined internal category")
    ...
    manager_fields.extend(['commission','seller_payment_limit','next_payment_request','auto_product_approve',
        'auto_approve_qty','internal_categ','location_id','warehouse_id','allow_product_variants',
        'total_mp_payment','paid_mp_payment','balance_mp_payment', 'state'])

    seller_fields.extend(['shipping_policy','return_policy','profile_msg'])

    seller_trackable_fields.extend(['state','commission','seller_payment_limit','next_payment_request'])
    ...
    def _calculate_mp_related_payment(self):
        for obj in filter(lambda e: e.seller, self):
            total_mp_payment = paid_mp_payment = cashable_amount = refund_amount = 0
            ...
            for seller_payment in seller_payment_objs:
                ...
                # Calculate marketplace refund payments for seller
                if seller_payment.state == 'confirm' and seller_payment.payment_mode == "order_refund":
                    refund_amount += abs(seller_payment.payable_amount)

            if refund_amount:
                total_mp_payment -= refund_amount
                cashable_amount -= refund_amount
    ...
    def on_change_seller_wise_settings(self):
        if not self.set_seller_wise_settings:
            ...
            vals["internal_categ"] = self.env['ir.default'].get(
                'res.config.settings', 'internal_categ')
            self.update(vals)
    ...
    def on_change_seller(self):
        ...
        self.internal_categ = config_setting_obj[
            "internal_categ"] if config_setting_obj.get("internal_categ") else False
    ...
    def get_seller_global_settings(self, config_setting_obj):
        ...
        internal_categ_id = config_setting_obj.get("internal_categ")
        internal_categ_id = [internal_categ_id] if internal_categ_id else []
        ...
        internal_categ_id = self.env["product.category"].browse(internal_categ_id)
        global_settings = {
            ...
            "internal_categ": internal_categ_id.name_get()[0] if internal_categ_id else None,
        }
    ...
    def _message_track_get_changes(self, tracked_fields, initial_values):
        if len(self) == 1 and not self.seller:
            tracked_fields = dict([(key, val) for key, val in tracked_fields.items() if key not in seller_trackable_fields])
        return super(ResPartner, self)._message_track_get_changes(tracked_fields, initial_values)
    ...
    def register_partner_as_a_seller(self):
        for rec in self:
            ...
            if current_user:
                ...
                raise Warning(_("Email is required for registering as a seller!"))
            return {
