...
class WebsiteDeals(models.Model):
	...
	dummy_pricelist = fields.Many2one('product.pricelist','Dummy Pricelist')
	...
	def get_dummy_pricelist(self):
		try:
			dummy_pricelist = self.env.ref("website_daily_deals.wk_deals_dummy_pricelist")
			if self.dummy_pricelist:
				self.dummy_pricelist.unlink()
			return dummy_pricelist
		except Exception as e:
			if not self.dummy_pricelist: 
				self.dummy_pricelist = self.env['product.pricelist'].create({
					"name":"Deals Dummy Pricelist (Do not delete/update me)",
					"discount_policy":"without_discount",
					"selectable":False,
					"sequence":1000,
				})
			return self.dummy_pricelist
	...
	def _update_deal_items(self):
		...
		if pricelist and self.state in ['validated','draft']:
			for item in self.pricelist_items:
				...
				if item.applied_on=='1_product':
					...
					cur = item.product_tmpl_id.currency_id
					a_price = cur._convert(item.product_tmpl_id.list_price, self.deal_pricelist.currency_id, self.env.company, fields.Date.today(), round=False)
				elif item.applied_on=='0_product_variant':
					d_price = pricelist.get_product_price(item.product_id,1,None)
					cur = item.product_id.currency_id
					a_price = cur._convert(item.product_id.lst_price, self.deal_pricelist.currency_id, self.env.company, fields.Date.today(), round=False)
				else:
					d_price = a_price = 0.0
			...
			for item in self.pricelist_items:
				item.pricelist_id = self.get_dummy_pricelist()
