...
class Website(models.Model):
	...
	def voucher_value_get_type(self):
		return self.env['ir.default'].sudo().get('res.config.settings', 'voucher_value_type')
	...
	def get_voucher_product_price(self, sale_order, voucher_obj, voucher_product_id):
		...
		order_total_price = 0
		amount_type = 'price_total' if self.voucher_value_get_type() == 'with_tax' else 'price_subtotal'
		for order_line_id in sale_order.order_line:
			if order_line_id.product_id.id != voucher_product_id and not order_line_id.is_delivery:
				order_total_price += getattr(order_line_id, amount_type)
				if voucher_obj and voucher_obj.applied_on == 'specific':
					if order_line_id.product_id.product_tmpl_id.id in voucher_obj.product_ids.ids:
						selected_prod_percent_price += getattr(order_line_id, amount_type)
	...
	def set_voucher_sale_order_price(self, sale_order, voucher_obj, voucher_product_id, selected_prod_percent_price,coupon_id, order_total_price):
		for order_line_id in sale_order.order_line:
			if order_line_id.product_id.id == voucher_product_id:
				...
				order_line_id.discount = 0
				# order_line_id.order_id.wk_voucher_value = order_line_id.price_total

				if hasattr(voucher_obj, 'validiate_gift_card') and hasattr(voucher_obj, 'gift_card_voucher'):
					if voucher_obj.gift_card_voucher:
						amount_dict = voucher_obj.validiate_gift_card()
						if amount_dict.get('amount'):
							voucher_value = amount_dict.get('amount')
							if order_total_price > voucher_value:
								order_line_id.price_unit = -voucher_value

				if order_line_id.price_unit >= 0 or (voucher_obj.use_minumum_cart_value and (sale_order.amount_total+sale_order.wk_coupon_value) < voucher_obj.minimum_cart_amount):
					order_line_id.unlink()
				history = self.env['voucher.history'].sudo().search([('sale_order_line_id','=',order_line_id.id)])
				if history:
					history = history[0]
					history.voucher_value = order_line_id.price_total
		return True
	...
	def get_remained_voucher_value(self, voucher_obj):
		...
		if voucher_obj.customer_type == 'special_customer':
			...
			if voucher_obj.is_partially_redemed:
				if voucher_obj.redeemption_limit == 1 or voucher_obj.voucher_val_type == 'percent':
